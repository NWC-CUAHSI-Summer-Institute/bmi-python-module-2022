...
class toybmi(Bmi):
    ...
    #---------------------------------------------
    _input_var_names = ['water_input', 'relative_humidity']
    ...
    _var_units_map = {  'water_input':'m/h',
                        'relative_humidity':'%',
                        'water_output':'m/h'
                        }
    ...
    def update_until(self, future_time: float):
        ...
        input = self._values["water_input"]
        loss_factor = self._values["loss_factor"]
        rh = self._values["relative_humidity"]

        # BEST WATER MODEL EVER
        self._values["water_output"][0] = input * ( ( 1.0 - rh ) * loss_factor )
